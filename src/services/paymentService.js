...
/**
 * Crear el RECARGA del conductor
 * @param {*} driverId Id driver
 * @param {*} newPayment
 * @param {*} listIds List de ids de viajes a cambir el estado como pagado
 * @returns 
 */
const createRecharge = async (driverId, newPayment, data) => {
  try {
    const listIds = data.paymentsList;
    const resState = await updateStatePaymentAll(driverId, listIds);
    if (!resState) {
    ...
    //discountType; // TODO se esta considerando solo por porcentaje
    ...
    newPayment.codPromo = promo.id ?? '';
    newPayment.paymentsList = listIds;
...
const createPayment = async (driverId, newPayment, data) => {
  try {
    const listIds = data.paymentsList;
    const resState = await updateStatePaymentAll(driverId, listIds);
    ...
    newPayment.credit = 0;
    newPayment.amount = totalCredit;
    newPayment.paymentsList = listIds;
    newPayment.promotionalPercentage = 0;
...
module.exports = { createPayment, createRecharge }